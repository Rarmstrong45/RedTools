...
export default function Login() {
  ...
  const { login, loginWithSSO, signup, isLoading } = useAuth();
      ...
      if (isSignUp) {
        ...
        if (success) {
          toast({
            ...
            description: "Welcome to RedTools!"
          });
        }
      } else {
  ...
  const handleSSO = async (provider: 'apple' | 'microsoft' | 'google') => {
    try {
      ...
      if (success) {
        ...
        toast({
          title: "Welcome!",
          description: `Logged in with ${providerNames[provider]}`
        });
        navigate('/');
      } else {
        toast({
      ...
      <Card className="w-full max-w-md bg-gray-900 border-gray-800">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center text-gray-400">
            {isSignUp 
              ? 'Sign up to get started with RedTools' 
              : 'Sign in to your RedTools account'
            }
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full bg-redtools-red hover:bg-redtools-red-dark text-white"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
